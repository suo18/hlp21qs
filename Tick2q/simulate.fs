...
module MapEnvt =
    ...
    let createEnv (initData: Variable list): Environment =
        ...
        let varName x = x.Name, x


        initData |> List.map varName |> Map.ofList
...
let step (envt: Environment): Environment =
    (envt, envt)
    ||> Map.fold (fun (env: Environment) (vName: string) (var: Variable) -> updateEnvt vName (var.UpdateFn envt) env)
...
let makeCktAdder numBits aPrefix bPrefix qPrefix cin cout =
    let rec makeAdderBit n =
        ...
        | n ->
            ...
            let coutN, sumN =
                fullAdder (makeVExp aPrefix) (makeVExp bPrefix) cin

            coutN,
            { Name = sprintf $"{qPrefix}{n}"
              UpdateFn = sumN
              Value = Ok Zero }
            :: lsOuts
...
let rec makeCktInputs (namePrefix: string) (numBits: uint32) (data: uint32) =
    ...
    | n ->
        ...
        let thisInput =
            makeCktVarBit namePrefix (n - 1u) (fun _env -> Ok dat) (getBit (n - 1u) data)
